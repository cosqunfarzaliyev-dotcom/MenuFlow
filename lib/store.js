...
import {
  ...
  handOverOrder as handOverOrderService,
  ...
  fetchPaymentIntegrationStatus as fetchPaymentIntegrationStatusService,
  savePaymentCredentials as savePaymentCredentialsService,
  disconnectPaymentIntegration as disconnectPaymentIntegrationService,
...
import { fetchSiteContent as fetchSiteContentService, resolveSiteText } from "@/lib/services/siteContentService";
...
export const useAppStore = create((set, get) => ({
  ...
  // Platform support contact — the "bizimlə əlaqə saxla" link the ADMIN panel
  // shows (trial banner, "əlavə işçi üçün dəstəyə yazın", and the subscription
  // lock screen). It is the same `site_content` row a super admin already edits
  // in SuperAdmin → Veb sayt → Əlaqə (`contact.whatsapp_url`, CONTACT_DETAIL_KEYS
  // in siteContentService.js). All three spots used to hardcode the seeded
  // placeholder literal, so editing it in SuperAdmin changed the marketing site
  // and nothing else — the admin panel kept dialing a dead number.
  //
  // The URL has no per-locale variant (defaults.js seeds az/en/ru identically),
  // so it resolves against 'az' and needs no language plumbing.
  supportContactUrl: SITE_CONTENT_DEFAULTS['contact.whatsapp_url']?.az || '',
  loadSupportContact: async () => {
    // fetchSiteContent() is unfiltered by design (see its comment) — one small
    // request for the whole table rather than a new one-key query shape.
    const rows = await fetchSiteContentService();
    const row = (rows || []).find((r) => r.key === 'contact.whatsapp_url');
    const url = resolveSiteText(row, 'az', get().supportContactUrl);
    set({ supportContactUrl: url });
    return url;
  },
  ...
  // Self-service handover (0046). One order: served + paid, one transaction —
  // at a counter those are the same moment, so splitting them into two calls
  // would leave a window where the food is gone but the money is not recorded.
  // Only StaffApp calls this, and only for a self_service restaurant.
  handOverOrder: async (orderId) => {
    const restaurantId = get().restaurant?.id;
    const { result, error } = await handOverOrderService(orderId);
    if (!error && result) {
      const orders = await fetchOrdersService(restaurantId);
      set({ orders });
      get().recordAudit({
        action: 'order.handed_over',
        entityType: 'order',
        entityId: orderId,
        summary: result.was_unpaid
          ? 'Sifariş təhvil verildi və ödəniş tamamlandı'
          : 'Sifariş təhvil verildi (artıq ödənilmişdi)',
      });
    }
    return { result, error };
  },
  ...
  // — posIntegration slice-ı ilə eyni forma, eyni "tək admin öz ayarını
  // redaktə edir, realtime lazım deyil" məntiqi.
  paymentIntegration: null,
  loadPaymentIntegration: async () => {
    const restaurantId = get().restaurant?.id;
    const paymentIntegration = await fetchPaymentIntegrationStatusService(restaurantId);
    set({ paymentIntegration });
    return paymentIntegration;
  },
  savePaymentIntegration: async ({ publicKey, privateKey, enabled }) => {
    const restaurantId = get().restaurant?.id;
    const { status, error } = await savePaymentCredentialsService({ restaurantId, publicKey, privateKey, enabled });
    if (!error && status) {
      set({ paymentIntegration: await fetchPaymentIntegrationStatusService(restaurantId) });
      // privateKey heç vaxt audit summary-yə düşmür — savePosIntegration ilə
      // eyni prinsip.
      get().recordAudit({
        action: 'payment_integration.update',
        entityType: 'payment_integration',
        entityId: restaurantId,
        summary: `Epoint ödəniş ayarları yeniləndi (${status.enabled ? 'aktiv' : 'deaktiv'})`,
      });
    }
    return { status, error };
  },
  disconnectPaymentIntegrationAction: async () => {
    const restaurantId = get().restaurant?.id;
    const { error } = await disconnectPaymentIntegrationService(restaurantId);
    if (!error) {
      set({ paymentIntegration: await fetchPaymentIntegrationStatusService(restaurantId) });
      get().recordAudit({
        ...
        entityType: 'payment_integration',
        entityId: restaurantId,
        summary: 'Epoint ödəniş bağlantısı kəsildi',
      });
    }
    return { error };
  },
