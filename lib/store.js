...
export const useAppStore = create((set, get) => ({
  ...
  createOrder: async ({ tableId, total, items, note }) => {
    const restaurantId = get().restaurant?.id;
    const { order, error } = await createOrderService({ tableId, total, items, note, restaurantId });
    if (!error && order) {
      const orders = await fetchOrdersService(restaurantId);
      set({ orders });
    }
  ...
  createAlert: async ({ tableId, type, paymentMethod, paymentMethodLabel, note }) => {
    const restaurantId = get().restaurant?.id;
    const { alert, error } = await createAlertService({ tableId, type, paymentMethod, paymentMethodLabel, note, restaurantId });
    if (!error && alert) {
      const alerts = await fetchAlertsService(restaurantId);
      set({ alerts });
    }
