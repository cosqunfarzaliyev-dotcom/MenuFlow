...
export const useAppStore = create(
  ...
  // 2. Supabase
  if (!supabaseReady) {
    console.warn("Supabase client is not ready; skipping remote order persistence.");
  } else {
  ...
  broadcastEvent(
    ...
    newOrder
      ...
      alerts: [], // { id, table, type: 'waiter'|'bill', paymentMethod, paymentMethodLabel, note, status: 'active'|'resolved', time }
      addAlert: (alert) => {
        const newAlert = {
          ...alert,
