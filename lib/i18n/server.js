...
// hreflang map for one marketing page, keyed by slug ('' = locale root).
//
// Next REPLACES `alternates` rather than merging it, so a sub-page that
// declares its own `alternates.languages` drops whatever the layout set —
// which is exactly how all five sub-pages silently shipped without the
// `x-default` entry the layout had. Every caller (the layout, the five
// pages, sitemap.js) goes through this one function so that can't recur:
// adding a locale or changing the fallback is a single edit here.
//
// x-default is the row Google serves to a visitor matching no listed
// language; it points at DEFAULT_LOCALE's copy, the same source the other
// two fall back through (lib/i18n/resolve.js).
export function localeAlternates(slug = '', origin = '') {
  const pathFor = (l) => `${origin}${slug ? `/${l}/${slug}` : `/${l}`}`;
  return {
