...
// ---------------------------------------------------------------------------
const handleQueryError = (functionName, error) => {
  const msg = error?.message || (typeof error === 'string' ? error : '');
  if (msg.includes('Failed to fetch') || msg.includes('fetch')) {
    console.warn(`[MenuFlow] ${functionName}: Supabase serverinə şəbəkə bağlantısı qurulmadı. Zəhmət olmasa terminalda 'npm run dev' serverini yenidən başladın və internet qoşulmasını/Supabase URL-ini yoxlayın.`);
  } else if (error?.code === '42P01' || msg.includes('does not exist')) {
    console.warn(`[MenuFlow] ${functionName}: Database table does not exist yet. Please run supabase/full_schema.sql in your Supabase SQL Editor.`);
  } else {
    console.error(`[MenuFlow] ${functionName} error:`, msg || error);
  }
};
...
export const fetchBanners = async (restaurantId, { activeOnly = false } = {}) => {
  ...
  if (error) {
    handleQueryError('fetchBanners', error);
    return [];
  }
...
export const fetchCampaigns = async (restaurantId) => {
  ...
  const { data, error } = await query;
  if (error) {
    handleQueryError('fetchCampaigns', error);
    return [];
  }
  return (data || []).map(normalizeRow);
};
