...
export const CAPABILITIES = {
  ...
  // Read-only staff directory (AdminApp's "İstifadəçilər" tab, 0028's
  // get_restaurant_staff() RPC) — deliberately separate from USERS_MANAGE
  // below. Viewing who already has an account is not the same trust level
  // as minting/removing one; keeping them as two keys means D1 (below) can
  // stay locked while still giving restaurant_admin a real, non-misleading
  // view of their own roster.
...
//     super-admin-only (`createOrAssignRestaurantUser`/`removeUserFromRestaurant`
//     are only called from `components/superadmin/RestaurantsTab.jsx`).
//     `users.view` is true, though — AdminApp's "İstifadəçilər" tab reads the
//     restaurant's own staff roster via get_restaurant_staff() (0028), a
//     narrow SECURITY DEFINER RPC with no create/delete/role-change path. If
//     D1 is ever revisited, `users.manage` is the one line that changes.
//   - `integrations.manage` (POS/Poster credentials, 0026_pos_integration.sql)
//     is kept as its OWN key rather than folded into `restaurant.settings`,
//     even though today both resolve identically (restaurant_admin: true,
//     staff: false) — a live third-party API token is a materially different
//     risk class from branding/table-count settings, and a future capability
//     split (e.g. a "manager" role that can edit branding but not
//     integrations) shouldn't require a schema-shaped refactor later.
const ROLE_CAPABILITIES = {
  ...
  staff: {
    ...
    [CAPABILITIES.USERS_VIEW]: false,
    [CAPABILITIES.USERS_MANAGE]: false,
    [CAPABILITIES.RESTAURANT_SETTINGS]: false,
    [CAPABILITIES.INTEGRATIONS]: false,
  },
};
