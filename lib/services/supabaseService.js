...
// session_generation (0051_table_session_generation.sql): bumped only by
// settle_table_payment(..., p_paid => true) — CustomerApp.jsx's "session
// expired" lock compares this against the generation it was locked at, so
// it stays a plain public column here rather than a separate RPC.
const PUBLIC_TABLE_COLUMNS = 'id, restaurant_id, table_number, name, created_at, session_generation';
...
// Ordered by the admin-controlled sort_order, NOT alphabetically. The
// column existed since 0001 and toCategoryRow could already write it, but
// nothing ever read it for ordering — so the Admin Panel's category order
// was alphabetical and unchangeable no matter what was stored. `name` stays
// as the tiebreaker, which keeps rows that were never explicitly reordered
// (all still at the default 0) in exactly the order they appear today.
...
// Self-service handover (0046_hand_over_order.sql): marks ONE order served and
// paid in a single server-side transaction, because at a self-service counter
// those are the same moment — staff hand the food over and take the money.
//
// Deliberately not settleTablePayment(): that one closes every unpaid order on
// the table at once, which is right when a waiter brings one bill, and wrong
// here — a second order from the same table would be marked paid while it is
// still in the kitchen. The RPC re-derives the restaurant from the order row
// itself, so no restaurantId is passed (or trusted).
export const handOverOrder = async (orderId) => {
  if (!supabaseReady) return { result: null, error: new Error('Supabase not ready') };
  const { data, error } = await supabase.rpc('hand_over_order', { p_order_id: orderId });
  if (error) {
...
// ---------------------------------------------------------------------------
// Epoint onlayn ödəniş inteqrasiyası (0048_epoint_payment_integration.sql).
// Yuxarıdakı üç POS funksiyası ilə eyni forma: get/upsert/disconnect,
// hər biri bir SECURITY DEFINER RPC-yə map olunur. private_key heç bir RPC
// nəticəsində qayıtmır — yalnız has_private_key boolean.
// ---------------------------------------------------------------------------
export const fetchPaymentIntegrationStatus = async (restaurantId) => {
  if (!supabaseReady) return null;
  const { data, error } = await supabase.rpc('get_payment_integration_status', { p_restaurant_id: restaurantId });
  if (error) {
    console.error('fetchPaymentIntegrationStatus error:', error);
    return null;
  }
  return data;
};

export const savePaymentCredentials = async ({ restaurantId, publicKey, privateKey, enabled }) => {
  if (!supabaseReady) return { status: null, error: new Error('Supabase not ready') };
  // privateKey undefined/'' ötürülürsə RPC-nin özü mövcud dəyəri saxlayır
  // (upsert_payment_credentials-in p_private_key default-u) — savePosCredentials
  // ilə eyni "yalnız toggle dəyişəndə sirr itməsin" məntiqi.
  const { data, error } = await supabase.rpc('upsert_payment_credentials', {
    p_restaurant_id: restaurantId,
  ...
  if (error) {
    console.error('savePaymentCredentials error:', error);
    return { status: null, error };
  }
  return { status: data, error: null };
};

export const disconnectPaymentIntegration = async (restaurantId) => {
  if (!supabaseReady) return { error: new Error('Supabase not ready') };
  const { error } = await supabase.rpc('disconnect_payment_integration', { p_restaurant_id: restaurantId });
  if (error) console.error('disconnectPaymentIntegration error:', error);
  return { error };
};

// Browser-invoked Edge Functions, both anon-callable (no session — the
// caller is an unauthenticated customer, see each function's own header).
// Same error.context unwrapping as syncPosMenuNow below: functions.invoke()
// discards the response body from error.message on any non-2xx.
const invokeEpointFunction = async (name, body, errorLabel) => {
  if (!supabaseReady) return { result: null, error: new Error('Supabase not ready') };
  ...
  if (error) {
    ...
    console.error(`${errorLabel} error:`, code, message);
...
// tableId/qrToken gate + server-computed amount happen entirely inside the
// Edge Function (epoint-create-payment) — this is a thin invoke wrapper.
// No returnUrl/language: the Apple Pay/Google Pay TOKEN WIDGET endpoint
// (1/token/widget) has no redirect concept at all — result.widgetUrl is
// embedded as an <iframe> inside CustomerApp.jsx's own bill modal, the
// customer never leaves the page. See epoint-create-payment/index.ts's
// header for the full protocol.
export const createEpointPayment = ({ restaurantId, tableId, qrToken }) =>
  invokeEpointFunction('epoint-create-payment', { restaurantId, tableId, qrToken }, 'createEpointPayment');

// Called repeatedly (polled) while the widget iframe above is open, not just
// once — epoint-confirm-payment is idempotent for exactly this reason (see
// its own header comment).
export const confirmEpointPayment = ({ transactionId }) =>
  invokeEpointFunction('epoint-confirm-payment', { transactionId }, 'confirmEpointPayment');
...
// Single pass, unlike siteContentService.reorderFaqItems: that one needs a
// negative scratch pass only because site_faq_items.sort_order carries a
// UNIQUE constraint (0032). categories.sort_order has no such index, so two
// rows may briefly share a value and a direct write is safe.
//
// supabase-js has no client-side transaction API, so this is a sequence of
// awaited updates rather than one atomic statement. Acceptable here for the
// same reason it is for the FAQ list: reordering is a low-frequency admin
// action, and a partial failure leaves a merely odd order, never corrupt
// data — the caller refetches, so whatever landed is what gets shown.
export const reorderCategories = async (idsInDesiredOrder) => {
  if (!supabaseReady) return { error: new Error('Supabase not ready') };
  try {
