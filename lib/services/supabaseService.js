...
const logSupabaseError = (context, error) => {
  try {
    const serialized = typeof error === 'object' ? JSON.stringify(error, Object.getOwnPropertyNames(error)) : String(error);
    console.error(`${context} error:`, serialized);
  } catch (e) {
...
export const fetchTables = async (restaurantId) => {
  ...
  if (error) {
    logSupabaseError('fetchTables', error);
    return [];
  }
...
export const fetchRestaurantQrTokens = async (restaurantId) => {
  ...
  if (error) {
    logSupabaseError('fetchRestaurantQrTokens', error);
    return {};
  }
  const byTableId = {};
  (data || []).forEach((row) => {
...
export const fetchTableByNumber = async (tableNumber, restaurantId) => {
  ...
  if (error) {
    logSupabaseError('fetchTableByNumber', error);
    return null;
  }
...
export const fetchOrders = async (restaurantId) => {
  ...
  const { data, error } = await query;
  if (error) {
    logSupabaseError('fetchOrders', error);
    return [];
  }
