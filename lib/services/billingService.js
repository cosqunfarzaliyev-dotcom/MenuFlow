...
// DEPRECATED (Master Plan D1, "Yol A" — super-admin-only account model):
// self-service restaurant creation is disabled. The underlying RPC is revoked
// from `authenticated` in 0018_disable_self_service_onboarding.sql, so this
// call now fails closed. Kept only so historical references don't break;
// no live caller remains (see app/onboarding/page.jsx). Restaurants are
// created by a super admin (superAdminService.createRestaurant), whose admin
// account is created in the same form (createOrAssignRestaurantUser, backed by
// the create-restaurant-user Edge Function). There is no public sign-up at all
// any more, so this function is doubly unreachable.
