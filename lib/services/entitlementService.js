...
export const FEATURES = {
  // Was two separate keys (APPLE_PAY/GOOGLE_PAY) until 0050_wallet_pay_
  // combined_feature.sql. Collapsed to one: the customer never sees two
  // separate wallet buttons (CustomerApp.jsx/CartDrawer.jsx render a single
  // device-labelled Epoint button), so two separate plan entitlements for it
  // never made sense once the old dual Payment-Request-API buttons were
  // removed. See that migration for the DB-side backfill.
...
export const FEATURE_REGISTRY = {
  [FEATURES.WALLET_PAY]: {
    ...
    description: 'Müştəri panelində Apple Pay / Google Pay ilə ödəniş',
    defaultEnabled: true,
    enforcement: 'ui',
  },
  [FEATURES.BANNERS]: {
