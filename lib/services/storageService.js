// ---------------------------------------------------------------------------
// Manual image upload (product/category/logo) — Supabase Storage.
// supabase/migrations/0033_media_uploads.sql creates the `restaurant-media`
// bucket (public read) and its RLS policies (write gated by is_admin_of(),
// keyed off the object path's first folder segment == restaurant_id). This
// file is the only thing in the app that talks to that bucket — every admin
// form (product/category image, restaurant logo) calls uploadRestaurantImage
// and just gets back a plain public URL, written into the same `image`/`logo`
// text columns a pasted external URL already went into. No new data shape,
// just a second way to fill it in.
// ---------------------------------------------------------------------------
...
// folder: 'products' | 'categories' | 'logo' — becomes the object path's
