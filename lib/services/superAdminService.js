...
import { DEFAULT_SERVICE_MODEL } from '@/lib/services/serviceModelService';
...
export const createRestaurant = async ({ slug, name, tagline, currencySymbol, tableCount, plan, serviceModel }) => {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      // 0045 — how the venue operates. Settable at creation (unlike `plan`,
      // which the edit form owns) because it's the first thing that shapes the
      // customer menu, and omitting it would silently start every new
      // restaurant as waiter-service.
      service_model: serviceModel || DEFAULT_SERVICE_MODEL,
    })
    .select('*')
    .single();
  if (error) {
...
export const updateRestaurant = async (restaurant) => {
  ...
  const {
    ...
    serviceModel, phone, address, onboardingCompletedAt,
  } = restaurant;
  const payload = {};
  ...
  // 0045 — the venue's operating model (replaced 0044's pay_later_enabled, which
  // it fully subsumes). Privileged: protect_restaurant_privileged_fields resets
  // it for anyone but a super_admin, same class as is_active/feature_flags above
  // rather than name/tagline below.
  if (serviceModel !== undefined) payload.service_model = serviceModel;
